...
class Database:
    ...
    def get_txs_from_address_in_range(
    ...
    def get_block(
        ...
        if option == self.BlockOption.EARLIEST:
            cmd = (
                f"""SELECT height, date, txs FROM blocks ORDER BY height ASC LIMIT 1"""
            )
        elif option == self.BlockOption.LATEST:
            cmd = (
...
if __name__ == "__main__":
    ...
    res = db.get_txs_from_address_in_range(
