...
def main():
    ...
    total: int = db.get_msg_type_count_in_range("*", earliest_block.height, earliest_block.height+1)
    print(f"Total Msgs: {total:,}")


    missing = db.get_missing_blocks(earliest_block.height, latest_block.height) # add 1 here to ensure it works and we actually miss blocks
    print(f"Missing Blocks total: {len(missing)}")


    txs_list = db.get_msg_types_transactions_in_range(
        "/cosmwasm.wasm.v1.MsgExecuteContract", earliest_block.height+1, earliest_block.height + 5
    )
    ...
    # get users txs
    txs: list[Tx] = db.get_users_txs_in_range("juno1nsnhn0y0vsjq8j70z6yfxp2xk4rsjrzmn04g4h", earliest_block.height, latest_block.height)
    print(f"User txs len: {len(txs)}")


    # Get a specific type of transaction over time
    specific_txs = db.get_msg_types_transactions_in_range("/cosmwasm.wasm.v1.MsgExecuteContract", earliest_block.height, latest_block.height)
    print(f"{specific_txs[0].id}, {specific_txs[0].height}, {specific_txs[0].msg_types}") # if using indexc 0, should be the first Tx indexed by earliest block height    
